...
function normalizeNumbersForSpeech(text: string): string {
  return text
    // currency symbols before amounts: £50k, $1.5M, €200
    ...
    // bare shorthand: 50k, 1.5M, 3B (not preceded by currency symbol — already handled above)
    ...
    // numeric ranges: 5-10, 3–7 (en-dash)
    .replace(/\b(\d+)\s*[-–]\s*(\d+)\b/g, "$1 to $2")
    // ordinals spoken naturally: 1st, 2nd, 3rd, 4th
