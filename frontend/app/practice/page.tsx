    ...
    <main className="min-h-screen bg-[#f4f8fc] dark:bg-[#0e1020]">
      ...
      <AnimatedPage className="mx-auto max-w-6xl px-4 py-16">
        <div className="mx-auto max-w-3xl text-center">
          <h1 className="text-5xl font-semibold tracking-[-0.045em] text-slate-900 dark:text-white md:text-6xl">Choose your pressure arena.</h1>
          <p className="mt-5 text-lg font-medium leading-8 text-slate-600 dark:text-white/60">Train reasoning, composure, communication intelligence, and recovery under realistic pressure.</p>
        </div>
          ...
          {practiceTypes.map((type) => (
            <AnimatedCard key={type} className="min-h-40 rounded-[1.5rem] bg-white p-6 shadow-[0_14px_40px_rgba(35,45,75,0.04)] ring-1 ring-slate-200/75 transition dark:bg-white/10 dark:ring-white/10">
              <Link href={`/practice/setup?type=${encodeURIComponent(type)}`} className="block h-full">
                ...
                <p className="mt-3 text-sm font-medium leading-6 text-slate-600 dark:text-white/60">Adaptive pressure, sharper follow-ups, and cognitive performance analytics.</p>
              </Link>
            </AnimatedCard>
          ))}
        </StaggeredGrid>
