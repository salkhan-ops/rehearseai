      ...
      <ProtectedRoute>
        <AnimatedPage className="mx-auto max-w-4xl px-4 py-14">
          ...
          <div className="space-y-3">
            {items.length === 0 ? <div className="rounded-[2rem] bg-white/75 p-8 text-center font-semibold text-slate-600 ring-1 ring-slate-200 dark:bg-white/[0.07] dark:text-white/58 dark:ring-white/10">No notifications yet.</div> : items.map((item) => (
              <div key={item.id} className="rounded-[1.5rem] bg-white/75 p-5 ring-1 ring-slate-200 dark:bg-white/[0.07] dark:ring-white/10">
                <div className="flex items-start justify-between gap-4">
                  ...
                  {!item.read && <button onClick={() => markRead(item.id)} aria-label="Mark as read" className="rounded-full bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700 dark:bg-white/10 dark:text-cyan-100"><Check size={14} /></button>}
                </div>
                {item.actionUrl && <Link href={item.actionUrl} className="mt-4 inline-flex rounded-full bg-slate-950 px-4 py-2 text-sm font-bold text-white dark:bg-white/10">Open</Link>}
              </div>
            ))}
          </div>
        </AnimatedPage>
