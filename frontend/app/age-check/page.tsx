...
function AgeCheckForm() {
  ...
  // A single checkbox stands in for all compliance facts (age, minor consent, terms,
  // privacy) -- see the matching consolidation in AuthForm.
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      await confirmAgeEligibility(consentAccepted);
    ...
    <main className="min-h-screen bg-[#f4f8fc] dark:bg-[#0e1020]">
      ...
      <AnimatedPage className="mx-auto max-w-xl px-4 py-16">
        <form onSubmit={submit} className="rounded-[1.75rem] bg-white p-7 shadow-[0_18px_55px_rgba(35,45,75,0.06)] ring-1 ring-slate-200/75 dark:bg-white/10 dark:ring-white/10">
          ...
          <div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-700 ring-1 ring-slate-200 dark:bg-white/[0.06] dark:text-white/72 dark:ring-white/10">
            <label className="flex items-start gap-3">
              <input type="checkbox" checked={consentAccepted} onChange={(event) => setConsentAccepted(event.target.checked)} className="mt-1 size-4 accent-[#6200a8]" />
              <span>
          ...
          <button disabled={saving || !consentAccepted} className="mt-6 w-full rounded-2xl bg-[#6200a8] px-5 py-3 font-semibold text-white shadow-[0_14px_30px_rgba(98,0,168,0.24)] disabled:opacity-60">
