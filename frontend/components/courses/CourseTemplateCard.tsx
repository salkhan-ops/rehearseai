...
export function CourseTemplateCard({ template, onStart }: { template: CourseTemplate; onStart: (template: CourseTemplate) => void }) {
  return (
    <article className="group relative overflow-hidden rounded-[2rem] bg-white/75 p-5 ring-1 ring-slate-200/80 backdrop-blur-xl transition hover:-translate-y-1 hover:ring-violet-200 dark:bg-white/[0.07] dark:ring-white/12">
      ...
      <div className="relative">
        ...
        <button onClick={() => onStart(template)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#6200a8] px-5 py-3 font-bold text-white shadow-[0_14px_34px_rgba(98,0,168,0.22)] transition hover:-translate-y-0.5">
