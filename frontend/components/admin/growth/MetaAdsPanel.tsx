...
function getMetricColumns(currency: string): { key: keyof MetaCreativeMetrics; label: string; format: (n: number) => string }[] {
  ...
  return [
    ...
    { key: "costPerLandingPageView", label: "Cost/LPV", format: money },
    { key: "purchases", label: "Purchases", format: (n) => n.toLocaleString() },
    { key: "costPerPurchase", label: "Cost/Purchase", format: money },
...
function MetricsTable({
  ...
  title: string;
  rows: ({ id: string; name: string } & MetaCreativeMetrics)[];
  columns: { key: keyof MetaCreativeMetrics; label: string; format: (n: number) => string }[];
}) {
  return (
...
export function MetaAdsPanel({ metaAds }: { metaAds: MetaAdsSnapshot }) {
  const money = formatMoney(metaAds.currency);
  const metricColumns = getMetricColumns(metaAds.currency);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-slate-500">
          {metaAds.isMock ? "Built to plug straight into the Meta Marketing API — every number below is illustrative until it's connected." : "Live data from the Meta Marketing API."}
        </p>
        {metaAds.isMock && <PreviewBadge />}
      </div>

      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
        <AdminStatCard icon={Award} label="Spend" value={money(metaAds.totals.spend)} />
        <AdminStatCard icon={Award} label="CTR" value={`${metaAds.totals.ctr.toFixed(2)}%`} />
        <AdminStatCard icon={Award} label="CPC" value={money(metaAds.totals.cpc)} />
        <AdminStatCard icon={Award} label="CPM" value={money(metaAds.totals.cpm)} />
        ...
        <AdminStatCard icon={Award} label="Cost / Landing Page View" value={money(metaAds.totals.costPerLandingPageView)} />
        <AdminStatCard icon={Award} label="Cost / Purchase" value={money(metaAds.totals.costPerPurchase)} />
      </div>

      <div className="grid gap-3 sm:grid-cols-3">
        <TrendChart title="Ad spend (30d)" data={metaAds.spendTrend} formatValue={money} isMock={metaAds.isMock} />
        <TrendChart title="CTR (30d)" data={metaAds.ctrTrend} formatValue={(n) => `${n.toFixed(2)}%`} color="#0ea5e9" isMock={metaAds.isMock} />
        <TrendChart title="CPC (30d)" data={metaAds.cpcTrend} formatValue={money} color="#f59e0b" isMock={metaAds.isMock} />
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-[1.25rem] bg-emerald-50 p-5 ring-1 ring-emerald-100">
          ...
          <div className="mt-1 text-sm font-medium text-slate-600">{metaAds.topCampaign.roas.toFixed(2)}x ROAS · {money(metaAds.topCampaign.spend)} spend</div>
        </div>
        <div className="rounded-[1.25rem] bg-emerald-50 p-5 ring-1 ring-emerald-100">
          ...
          <div className="mt-1 text-sm font-medium text-slate-600">{metaAds.topAd.roas.toFixed(2)}x ROAS · {money(metaAds.topAd.costPerPurchase)}/purchase</div>
        </div>
        <div className="rounded-[1.25rem] bg-rose-50 p-5 ring-1 ring-rose-100">
          ...
          <div className="mt-1 text-sm font-medium text-slate-600">{metaAds.worstAd.roas.toFixed(2)}x ROAS · {money(metaAds.worstAd.costPerPurchase)}/purchase</div>
        </div>
      </div>

      <MetricsTable title="Campaigns" rows={metaAds.campaigns} columns={metricColumns} />
      <MetricsTable title="Ad sets" rows={metaAds.adSets} columns={metricColumns} />
      <MetricsTable title="Ads" rows={metaAds.ads} columns={metricColumns} />
    </div>
  );
}
