...
async def synthesize_voice(payload: TTSRequest, request: Request):
    try:
        audio, content_type = await request.app.state.cartesia.synthesize(payload.text)
...
async def connect_to_deepgram(api_key: str):
    headers = {"Authorization": f"Token {api_key}"}
    try:
        return await websockets.connect(DEEPGRAM_LISTEN_URL, additional_headers=headers)
    except TypeError:
        return await websockets.connect(DEEPGRAM_LISTEN_URL, extra_headers=headers)
...
async def deepgram_voice_proxy(websocket: WebSocket):
    ...
    deepgram = None
    try:
        deepgram = await connect_to_deepgram(api_key)
        await websocket.send_json({"type": "proxy_ready", "provider": "deepgram"})
