...
MODEL_PATH = ROOT / "models" / "user_state_classifier.joblib"
...
def main() -> None:
    ...
    rows = [row for row in load_rows(Path(args.data)) if row.get("userState") or row.get("detectedUserState")]
    for row in rows:
    ...
    x = vectorize(rows)
    y = [row["userState"] for row in rows]
